...
# Main admin user ID (Updated)
MAIN_ADMIN_ID = 1427636685595742260
# VPS User Role ID (Updated)
VPS_USER_ROLE_ID = 1310222158726959235
...
def load_admin_data():
    try:
        ...
        return {"admins": [str(MAIN_ADMIN_ID), "1310222158726959235"]}
...
def is_admin():
    async def predicate(ctx):
        ...
        # Check if user is main admin OR in admin list
        if user_id == str(MAIN_ADMIN_ID) or user_id == "1310222158726959235" or user_id in admin_data.get("admins", []):
            return True
...
def is_main_admin():
    async def predicate(ctx):
        user_id = str(ctx.author.id)
        # Only the MAIN_ADMIN_ID can use main admin commands
...
def create_embed(title, description="", color=0x1a1a1a, fields=None):
    ...
    embed.set_footer(
        text=f"Axzahost | VPS Manager • {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        icon_url=""
    )
    return embed
...
async def on_ready():
    ...
    await bot.change_presence(activity=discord.Activity(
        type=discord.ActivityType.watching, name="Axzahost | VPS Manager"))
...
async def manage_vps(ctx, user: discord.Member = None):
    ...
    if user:
        if not (str(ctx.author.id) == str(MAIN_ADMIN_ID) or str(ctx.author.id) == "1310222158726959235" or str(ctx.author.id) in admin_data.get("admins", [])):
...
async def show_plans(ctx):
    ...
    embed = create_embed("💎 VPS Plans - Axzahost", "Choose your perfect VPS plan:", 0x1a1a1a)
    plans_info = [
        ("🥉 Starter", "**RAM:** 4GB\n**CPU:** 1 Core\n**Storage:** 10GB\n**Intel:** 42 credits\n**AMD:** 83 credits"),
        ("🥈 Basic", "**RAM:** 8GB\n**CPU:** 1 Core\n**Storage:** 10GB\n**Intel:** 96 credits\n**AMD:** 164 credits"),
        ("🥇 Standard", "**RAM:** 12GB\n**CPU:** 2 Cores\n**Storage:** 10GB\n**Intel:** 192 credits\n**AMD:** 320 credits"),
...
async def user_info(ctx, user: discord.Member):
    ...
    is_admin_user = user_id == str(MAIN_ADMIN_ID) or user_id == "1310222158726959235" or user_id in admin_data.get("admins", [])
...
async def show_help(ctx):
    ...
    user_id = str(ctx.author.id)
    is_user_admin = user_id == str(MAIN_ADMIN_ID) or user_id == "1310222158726959235" or user_id in admin_data.get("admins", [])
...
async def bot_status(ctx):
    ...
    total_vps = sum(len(v) for v in vps_data.values())
    running_vps = sum(1 for vl in vps_data.values() for v in vl if v.get("status") == "running")
    embed = create_embed("🤖 Bot Status", "Axzahost VPS Manager", 0x00ff88)
...
async def announce(ctx, *, message: str):
    ...
    announce_embed.add_field(name="From", value=f"**Axzahost Team** ({ctx.author.mention})", inline=False)
...
async def maintenance_toggle(ctx, mode: str):
    ...
    if mode.lower() == "on":
        ...
        await bot.change_presence(
            ...
            activity=discord.Activity(type=discord.ActivityType.watching, name="Axzahost | VPS Manager")
        )
...
async def on_message(message):
    ...
    if isinstance(message.channel, discord.DMChannel):
        if message.content.startswith(bot.command_prefix):
            ...
            block_embed.set_footer(text="Axzahost | Server-only bot")
...
async def stats_alias(ctx):
    if str(ctx.author.id) == str(MAIN_ADMIN_ID) or str(ctx.author.id) == "1310222158726959235" or str(ctx.author.id) in admin_data.get("admins", []):
...
async def check_expire(ctx, user: discord.Member = None):
    ...
    is_user_admin = str(ctx.author.id) == str(MAIN_ADMIN_ID) or str(ctx.author.id) == "1310222158726959235" or str(ctx.author.id) in admin_data.get("admins", [])
